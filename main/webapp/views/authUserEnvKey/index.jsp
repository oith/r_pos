                ...
                <div class='box box-primary'>   
                    ...
                    <div class='box-body'>
                        <table class='table dt-default table-bordered table-striped table-hover table-condensed display nowrap th-center'>
                            ...
                            <thead>
                                <th><spring:message code='authUser' text='Auth User'/></th>
                            ...
                            <tbody>
                                <c:if test='${not empty authUserEnvKeys}'>
                                    <spring:message code='default.button.delete.confirm.message' text='Are you sure to delete?' var='confirmToDelete'/>
                                    <c:forEach items='${authUserEnvKeys}' var='authUserEnvKey'  varStatus='loopStatus'>
                                        <tr class='${loopStatus.index % 2 == 0 ? 'odd' : 'even'}'>
                                            <td><c:out value='${authUserEnvKey.authUser!=null ? authUserEnvKey.authUser :"N/A"}'/></td>
                                            <td class='center'><c:out value='${authUserEnvKey.envKey}'/></td>
                                            <td><c:out value='${authUserEnvKey.envValue}'/></td>

                                            <td class='center action'>
                                                <sec:authorize url='/authUserEnvKey/show'>
                                                    <a class='btn btn-info btn-xs' href='${context}/authUserEnvKey/show/${authUserEnvKey.id}'>
                                                        <i class='fa fa-info-circle'></i>
                                                        <spring:message code='default.button.show.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/authUserEnvKey/edit'>
                                                    <a class='btn btn-primary btn-xs' href='${context}/authUserEnvKey/edit/${authUserEnvKey.id}'>
                                                        <i class='fa fa-edit'></i>
                                                        <spring:message code='default.button.edit.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/authUserEnvKey/copy'>
                                                    <a class='btn btn-warning btn-xs' href='${context}/authUserEnvKey/copy/${authUserEnvKey.id}'>
                                                        <i class='fa fa-clone' aria-hidden='true'></i>
                                                        <spring:message code='default.button.copy.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/authUserEnvKey/delete'>
                                                    <a class='btn btn-danger btn-xs' href='${context}/authUserEnvKey/delete/${authUserEnvKey.id}' onclick="return confirm('${confirmToDelete}');">
