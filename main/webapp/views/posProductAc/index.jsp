                ...
                <div class='box box-primary'>   
                    ...
                    <div class='box-body'>
                        <table class='table dt-default table-bordered table-striped table-hover table-condensed display nowrap th-center'>
                            ...
                            <tbody>
                                <c:if test='${not empty posProductAcs}'>
                                    <spring:message code='default.button.delete.confirm.message' text='Are you sure to delete?' var='confirmToDelete'/>
                                    <c:forEach items='${posProductAcs}' var='posProductAc'  varStatus='loopStatus'>
                                        <tr class='${loopStatus.index % 2 == 0 ? 'odd' : 'even'}'>
                                            <td class='center'><c:out value='${posProductAc.code}'/></td>
                                            <td><c:out value='${posProductAc.fullName}'/></td>
                                            <td><c:out value='${posProductAc.fullNameNative}'/></td>
                                            <td class='center'><c:out value='${posProductAc.emPosAnalysisCode}'/></td>

                                            <td class='center action'>
                                                <sec:authorize url='/posProductAc/show'>
                                                    <a class='btn btn-info btn-xs' href='${context}/posProductAc/show/${posProductAc.id}'>
                                                        <i class='fa fa-info-circle'></i>
                                                        <spring:message code='default.button.show.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/posProductAc/edit'>
                                                    <a class='btn btn-primary btn-xs' href='${context}/posProductAc/edit/${posProductAc.id}'>
                                                        <i class='fa fa-edit'></i>
                                                        <spring:message code='default.button.edit.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/posProductAc/copy'>
                                                    <a class='btn btn-warning btn-xs' href='${context}/posProductAc/copy/${posProductAc.id}'>
                                                        <i class='fa fa-clone' aria-hidden='true'></i>
                                                        <spring:message code='default.button.copy.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/posProductAc/delete'>
                                                    <a class='btn btn-danger btn-xs' href='${context}/posProductAc/delete/${posProductAc.id}' onclick="return confirm('${confirmToDelete}');">
