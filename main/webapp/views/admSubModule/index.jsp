                    ...
                    <div class='box-body'>
                        <table class='table dt-default table-bordered table-striped table-hover table-condensed display nowrap th-center'>
                            ...
                            <thead>
                                ...
                                <th><spring:message code='admModule' text='Adm Module'/></th>
                            ...
                            <tbody>
                                <c:if test='${not empty admSubModules}'>
                                    <spring:message code='default.button.delete.confirm.message' text='Are you sure to delete?' var='confirmToDelete'/>
                                    <c:forEach items='${admSubModules}' var='admSubModule'  varStatus='loopStatus'>
                                        <tr class='${loopStatus.index % 2 == 0 ? 'odd' : 'even'}'>
                                            <td><c:out value='${admSubModule.code}'/></td>
                                            <td><c:out value='${admSubModule.fullName}'/></td>
                                            <td><c:out value='${admSubModule.fullNameNative}'/></td>
                                            <td class='center'><c:if test='${admSubModule.isActive}'><spring:message code='default.boolean.true' text='YES'/></c:if><c:if test='${!admSubModule.isActive}'><spring:message code='default.boolean.false' text='NO'/></c:if></td>
                                            <td class='right'><c:out value='${admSubModule.slNo}'/></td>
                                            <td><c:out value='${admSubModule.description}'/></td>
                                            <td><c:out value='${admSubModule.admModule}'/></td>
                                            <td><c:out value='${admSubModule.displayIconClass}'/></td>

                                            <td class='center action'>
                                                <sec:authorize url='/admSubModule/show'>
                                                    <a class='btn btn-info btn-xs' href='${context}/admSubModule/show/${admSubModule.id}'>
                                                        <i class='fa fa-info-circle'></i>
                                                        <spring:message code='default.button.show.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/admSubModule/edit'>
                                                    <a class='btn btn-primary btn-xs' href='${context}/admSubModule/edit/${admSubModule.id}'>
                                                        <i class='fa fa-edit'></i>
                                                        <spring:message code='default.button.edit.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/admSubModule/copy'>
                                                    <a class='btn btn-warning btn-xs' href='${context}/admSubModule/copy/${admSubModule.id}'>
                                                        <i class='fa fa-clone' aria-hidden='true'></i>
                                                        <spring:message code='default.button.copy.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/admSubModule/delete'>
                                                    <a class='btn btn-danger btn-xs' href='${context}/admSubModule/delete/${admSubModule.id}' onclick="return confirm('${confirmToDelete}');">
