                ...
                <div class='box box-primary'>   
                    ...
                    <div class='box-body'>
                        <table class='table dt-default table-bordered table-striped table-hover table-condensed display nowrap th-center'>
                            ...
                            <thead>
                                <th><spring:message code='choose' text='Choose'/></th>
                                <th><spring:message code='isActive' text='Is Active'/></th>

                                <th><spring:message code='default.button.action.label' text='Action'/></th> 
                            </thead>
                            <tbody>
                                <c:if test='${not empty zxChooses}'>
                                    <spring:message code='default.button.delete.confirm.message' text='Are you sure to delete?' var='confirmToDelete'/>
                                    <c:forEach items='${zxChooses}' var='zxChoose'  varStatus='loopStatus'>
                                        <tr class='${loopStatus.index % 2 == 0 ? 'odd' : 'even'}'>
                                            <td><c:out value='${zxChoose.choose}'/></td>
                                            <td class='center'><c:if test='${zxChoose.isActive}'><spring:message code='default.boolean.true' text='YES'/></c:if><c:if test='${!zxChoose.isActive}'><spring:message code='default.boolean.false' text='NO'/></c:if></td>

                                            <td class='center action'>
                                                <sec:authorize url='/zxChoose/show'>
                                                    <a class='btn btn-info btn-xs' href='${context}/zxChoose/show/${zxChoose.id}'>
                                                        <i class='fa fa-info-circle'></i>
                                                        <spring:message code='default.button.show.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/zxChoose/edit'>
                                                    <a class='btn btn-primary btn-xs' href='${context}/zxChoose/edit/${zxChoose.id}'>
                                                        <i class='fa fa-edit'></i>
                                                        <spring:message code='default.button.edit.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/zxChoose/copy'>
                                                    <a class='btn btn-warning btn-xs' href='${context}/zxChoose/copy/${zxChoose.id}'>
                                                        <i class='fa fa-clone' aria-hidden='true'></i>
                                                        <spring:message code='default.button.copy.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/zxChoose/delete'>
                                                    <a class='btn btn-danger btn-xs' href='${context}/zxChoose/delete/${zxChoose.id}' onclick="return confirm('${confirmToDelete}');">
