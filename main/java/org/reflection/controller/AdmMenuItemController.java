...
import org.reflection.service.AdmMenuService;
...
@Controller
@RequestMapping(value = "/admMenuItem")
@SessionAttributes({"admSubModules"})
public class AdmMenuItemController extends _BaseController {

    protected static final String MODEL = "admMenuItem";
    ...
    String getCodableDTO(@RequestParam(value = "code") String code) {
        try {
            AdmMenuItem codable = admMenuItemService.findByCode(code);
            return codable.getFullName();
        } catch (Exception e) {
            System.out.println("mac err codable AdmMenuItem Not Found: " + e);
            return "Not Found";
        }
    }
    ...
    private void commonPost(AdmMenuItem currObject) {
        try {
            currObject.setAdmMenu(admMenuService.findByCode(currObject.getAdmMenu().getCode()));
        } catch (Exception e) {
            currObject.setAdmMenu(null);
        }
    ...
    public String create(ModelMap model) {
        model.addAttribute(MODEL, new AdmMenuItem());
    ...
    @PostMapping("/create")
    public String save(
            @ModelAttribute(MODEL) @Valid AdmMenuItem currObject,
        ...
        if (!bindingResult.hasErrors()) {
            try {
                AdmMenuItem admMenuItem = admMenuItemService.create(currObject);
                admMenuItem.setId(faker(admMenuItem.getId()));
                addFeedbackMessage(redirectAttributes, FLASH_SUCCESS, FEEDBACK_MESSAGE_KEY_CREATED, admMenuItem.getCode());
                return "redirect:/" + SHOW + "/" + admMenuItem.getId();
            } catch (Exception e) {
    ...
    public String edit(@PathVariable("id") BigInteger id, ModelMap model, RedirectAttributes redirectAttributes) {
        ...
        try {
            AdmMenuItem admMenuItem = admMenuItemService.findById(faker(id));
            admMenuItem.setId(faker(admMenuItem.getId()));
            model.addAttribute(MODEL, admMenuItem);
            model.addAttribute("_action", "edit");
            return FORM;
        } catch (AdmMenuItemNotFoundException e) {
    ...
    public String update(
        ...
        if (!bindingResult.hasErrors()) {
            try {
                AdmMenuItem admMenuItem = admMenuItemService.update(currObject);
                addFeedbackMessage(redirectAttributes, FLASH_SUCCESS, FEEDBACK_MESSAGE_KEY_EDITED, admMenuItem.getCode());
    ...
    public String copied(
        ...
        if (!bindingResult.hasErrors()) {
            try {
                AdmMenuItem admMenuItem = admMenuItemService.copy(currObject);
                admMenuItem.setId(faker(admMenuItem.getId()));
                addFeedbackMessage(redirectAttributes, FLASH_SUCCESS, FEEDBACK_MESSAGE_KEY_COPIED, admMenuItem.getCode());
                return "redirect:/" + SHOW + "/" + admMenuItem.getId();
            } catch (Exception e) {
                errorHandler(bindingResult, e);
            }
    ...
    public String search(@ModelAttribute(SEARCH_CRITERIA) _SearchDTO searchCriteria, ModelMap model) {
        ...
        List<AdmMenuItem> admMenuItems;
   
        if (searchTerm != null && !searchTerm.trim().isEmpty()) {
            admMenuItems = admMenuItemService.search(searchCriteria);
        } else {
            admMenuItems = admMenuItemService.findAll(searchCriteria);
        }
        model.addAttribute(MODELS, admMenuItems);
        ...
        */
        Iterable<AdmMenuItem> admMenuItems = admMenuItemService.findAll();
        for (AdmMenuItem obj : admMenuItems) {
    ...
    public String index(ModelMap model) {
        ...
        List<AdmMenuItem> admMenuItems = admMenuItemService.findAll(searchCriteria);
    ...
    public String delete(@PathVariable("id") BigInteger id, RedirectAttributes redirectAttributes) {
        ...
        try {
            AdmMenuItem deleted = admMenuItemService.delete(faker(id));
