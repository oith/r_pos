...
@Controller
@RequestMapping(value = "/admMenu")
@SessionAttributes({"admSubModules"})
public class AdmMenuController extends _BaseController {

    protected static final String MODEL = "admMenu";
    ...
    String getCodableDTO(@RequestParam(value = "code") String code) {
        try {
            AdmMenu codable = admMenuService.findByCode(code);
            return codable.getFullName();
        } catch (Exception e) {
            System.out.println("mac err codable AdmMenu Not Found: " + e);
            return "Not Found";
        }
    }
    ...
    private void commonPost(AdmMenu currObject) {
    ...
    public String create(ModelMap model) {
        model.addAttribute(MODEL, new AdmMenu());
    ...
    @PostMapping("/create")
    public String save(
            @ModelAttribute(MODEL) @Valid AdmMenu currObject,
        ...
        if (!bindingResult.hasErrors()) {
            try {
                AdmMenu admMenu = admMenuService.create(currObject);
                admMenu.setId(faker(admMenu.getId()));
                addFeedbackMessage(redirectAttributes, FLASH_SUCCESS, FEEDBACK_MESSAGE_KEY_CREATED, admMenu.getCode());
                return "redirect:/" + SHOW + "/" + admMenu.getId();
            } catch (Exception e) {
    ...
    public String edit(@PathVariable("id") BigInteger id, ModelMap model, RedirectAttributes redirectAttributes) {
        ...
        try {
            AdmMenu admMenu = admMenuService.findById(faker(id));
            admMenu.setId(faker(admMenu.getId()));
            model.addAttribute(MODEL, admMenu);
            model.addAttribute("_action", "edit");
            return FORM;
        } catch (AdmMenuNotFoundException e) {
    ...
    public String update(
        ...
        if (!bindingResult.hasErrors()) {
            try {
                AdmMenu admMenu = admMenuService.update(currObject);
                addFeedbackMessage(redirectAttributes, FLASH_SUCCESS, FEEDBACK_MESSAGE_KEY_EDITED, admMenu.getCode());
    ...
    public String copied(
        ...
        if (!bindingResult.hasErrors()) {
            try {
                AdmMenu admMenu = admMenuService.copy(currObject);
                admMenu.setId(faker(admMenu.getId()));
                addFeedbackMessage(redirectAttributes, FLASH_SUCCESS, FEEDBACK_MESSAGE_KEY_COPIED, admMenu.getCode());
                return "redirect:/" + SHOW + "/" + admMenu.getId();
            } catch (Exception e) {
                errorHandler(bindingResult, e);
            }
    ...
    public String search(@ModelAttribute(SEARCH_CRITERIA) _SearchDTO searchCriteria, ModelMap model) {
        ...
        List<AdmMenu> admMenus;
   
        if (searchTerm != null && !searchTerm.trim().isEmpty()) {
            admMenus = admMenuService.search(searchCriteria);
        } else {
            admMenus = admMenuService.findAll(searchCriteria);
        }
        model.addAttribute(MODELS, admMenus);
        ...
        */
        Iterable<AdmMenu> admMenus = admMenuService.findAll();
        for (AdmMenu obj : admMenus) {
    ...
    public String index(ModelMap model) {
        ...
        List<AdmMenu> admMenus = admMenuService.findAll(searchCriteria);
    ...
    public String delete(@PathVariable("id") BigInteger id, RedirectAttributes redirectAttributes) {
        ...
        try {
            AdmMenu deleted = admMenuService.delete(faker(id));
