                    ...
                    <div class='box-body'>
                        <table class='table dt-default table-bordered table-striped table-hover table-condensed display nowrap th-center'>
                            ...
                            <thead>
                                ...
                                <th><spring:message code='lookupKeyword' text='Lookup Keyword'/></th>
                            ...
                            <tbody>
                                <c:if test='${not empty admLookups}'>
                                    <spring:message code='default.button.delete.confirm.message' text='Are you sure to delete?' var='confirmToDelete'/>
                                    <c:forEach items='${admLookups}' var='admLookup'  varStatus='loopStatus'>
                                        <tr class='${loopStatus.index % 2 == 0 ? 'odd' : 'even'}'>
                                            <td><c:out value='${admLookup.code}'/></td>
                                            <td><c:out value='${admLookup.fullName}'/></td>
                                            <td><c:out value='${admLookup.fullNameNative}'/></td>
                                            <td class='center'><c:out value='${admLookup.lookupKeyword}'/></td>
                                            <td class='center'><c:if test='${admLookup.isActive}'><spring:message code='default.boolean.true' text='YES'/></c:if><c:if test='${!admLookup.isActive}'><spring:message code='default.boolean.false' text='NO'/></c:if></td>
                                            <td class='right'><c:out value='${admLookup.slNo}'/></td>
                                            <td><c:out value='${admLookup.remarks}'/></td>

                                            <td class='center action'>
                                                <sec:authorize url='/admLookup/show'>
                                                    <a class='btn btn-info btn-xs' href='${context}/admLookup/show/${admLookup.id}'>
                                                        <i class='fa fa-info-circle'></i>
                                                        <spring:message code='default.button.show.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/admLookup/edit'>
                                                    <a class='btn btn-primary btn-xs' href='${context}/admLookup/edit/${admLookup.id}'>
                                                        <i class='fa fa-edit'></i>
                                                        <spring:message code='default.button.edit.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/admLookup/copy'>
                                                    <a class='btn btn-warning btn-xs' href='${context}/admLookup/copy/${admLookup.id}'>
                                                        <i class='fa fa-clone' aria-hidden='true'></i>
                                                        <spring:message code='default.button.copy.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/admLookup/delete'>
                                                    <a class='btn btn-danger btn-xs' href='${context}/admLookup/delete/${admLookup.id}' onclick="return confirm('${confirmToDelete}');">
