                                ...
                                <c:if test='${not empty authRoles}'>
                                    ...
                                    <c:forEach items='${authRoles}' var='authRole'  varStatus='loopStatus'>
                                        <tr class='${loopStatus.index % 2 == 0 ? 'odd' : 'even'}'>
                                            ...
                                            <td class='center'><c:if test='${authRole.isActive}'><spring:message code='default.boolean.true' text='YES'/></c:if><c:if test='${!authRole.isActive}'><spring:message code='default.boolean.false' text='NO'/></c:if></td>

                                            <td class='center action'>
                                                <sec:authorize url='/authRole/show'>
                                                    <a class='btn btn-info btn-xs' href='${context}/authRole/show/${authRole.id}'>
                                                        <i class='fa fa-info-circle'></i>
                                                        <spring:message code='default.button.show.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/authRole/edit'>
                                                    <a class='btn btn-primary btn-xs' href='${context}/authRole/edit/${authRole.id}'>
                                                        <i class='fa fa-edit'></i>
                                                        <spring:message code='default.button.edit.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/authRole/copy'>
                                                    <a class='btn btn-warning btn-xs' href='${context}/authRole/copy/${authRole.id}'>
                                                        <i class='fa fa-clone' aria-hidden='true'></i>
                                                        <spring:message code='default.button.copy.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/authRole/delete'>
                                                    <a class='btn btn-danger btn-xs' href='${context}/authRole/delete/${authRole.id}' onclick="return confirm('${confirmToDelete}');">
