...
<spring:message code='admProcesss' text='Adm Processs'/> | <a href='${context}/admProcess/create'><spring:message code='create.link.label'/>&NonBreakingSpace;<spring:message code='AdmProcess' text='Adm Process'/></a>
            ...
            <tbody>
                ...
                <c:forEach items='${admModule.admProcesss}' var='admProcesss'  varStatus='loopStatus'>
                    <tr class='${loopStatus.index % 2 == 0 ? 'odd' : 'even'}'>
                        <td><a href='${context}/admProcess/show/<c:out value='${admProcesss.id}'/>'><spring:message code='show.link.label'/></a></td>
                        ...
                        <td class='center'><c:if test='${admProcesss.isActive}'><spring:message code='default.boolean.true' text='YES'/></c:if><c:if test='${!admProcesss.isActive}'><spring:message code='default.boolean.false' text='NO'/></c:if></td>
                        ...
                        <td><a href='${context}/admProcess/edit/<c:out value='${admProcesss.id}'/>'><spring:message code='edit.link.label'/></a></td>
                        <td><a href='${context}/admProcess/delete/<c:out value='${admProcesss.id}'/>' onclick="return confirm('${confirmToDelete}');"><spring:message code='delete.link.label'/></a></td>
                    </tr>
                </c:forEach>
            </tbody>
        </table>
