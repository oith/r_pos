                    ...
                    <div class='box-body'>
                        <table class='table dt-default table-bordered table-striped table-hover table-condensed display nowrap th-center'>
                            ...
                            <thead>
                                ...
                                <th><spring:message code='admReport' text='Adm Report'/></th>
                                ...
                                <c:if test='${not empty admReportDetails}'>
                                    ...
                                    <c:forEach items='${admReportDetails}' var='admReportDetail'  varStatus='loopStatus'>
                                        <tr class='${loopStatus.index % 2 == 0 ? 'odd' : 'even'}'>
                                            <td class='right'><c:out value='${admReportDetail.slNo}'/></td>
                                            <td><c:out value='${admReportDetail.admReport!=null ? admReportDetail.admReport :"N/A"}'/></td>
                                            <td><c:out value='${admReportDetail.admParam}'/></td>
                                            <td class='center'><c:if test='${admReportDetail.isActive}'><spring:message code='default.boolean.true' text='YES'/></c:if><c:if test='${!admReportDetail.isActive}'><spring:message code='default.boolean.false' text='NO'/></c:if></td>
                                            <td class='center'><c:if test='${admReportDetail.isMandatory}'><spring:message code='default.boolean.true' text='YES'/></c:if><c:if test='${!admReportDetail.isMandatory}'><spring:message code='default.boolean.false' text='NO'/></c:if></td>
                                            <td><c:out value='${admReportDetail.defaultVal}'/></td>
                                            <td><c:out value='${admReportDetail.helpText}'/></td>

                                            <td class='center action'>
                                                <sec:authorize url='/admReportDetail/show'>
                                                    <a class='btn btn-info btn-xs' href='${context}/admReportDetail/show/${admReportDetail.id}'>
                                                        <i class='fa fa-info-circle'></i>
                                                        <spring:message code='default.button.show.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/admReportDetail/edit'>
                                                    <a class='btn btn-primary btn-xs' href='${context}/admReportDetail/edit/${admReportDetail.id}'>
                                                        <i class='fa fa-edit'></i>
                                                        <spring:message code='default.button.edit.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/admReportDetail/copy'>
                                                    <a class='btn btn-warning btn-xs' href='${context}/admReportDetail/copy/${admReportDetail.id}'>
                                                        <i class='fa fa-clone' aria-hidden='true'></i>
                                                        <spring:message code='default.button.copy.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/admReportDetail/delete'>
                                                    <a class='btn btn-danger btn-xs' href='${context}/admReportDetail/delete/${admReportDetail.id}' onclick="return confirm('${confirmToDelete}');">
