...
<spring:message code='admProcessDetails' text='Adm Process Details'/> | <a href='${context}/admProcessDetail/create'>
    <spring:message code='default.create.link.label'/>&NonBreakingSpace;<spring:message code='AdmProcessDetail'
                                                                                        text='Adm Process Detail'/></a>
        ...
        <table class='table table-bordered table-striped table-hover table-condensed dt-responsive display nowrap th-center'>
            ...
            <c:forEach items='${admProcess.admProcessDetails}' var='admProcessDetails' varStatus='loopStatus'>
                <tr class='${loopStatus.index % 2 == 0 ? 'odd' : 'even'}'>
                    <td>
                        <a href='${context}/admProcessDetail/show/<c:out value='${admProcessDetails.id}'/>'><spring:message
                    ...
                    <td class='center'><c:if test='${admProcessDetails.isActive}'><spring:message
                            code='default.boolean.true' text='YES'/></c:if><c:if
                            test='${!admProcessDetails.isActive}'><spring:message code='default.boolean.false'
                                                                                  text='NO'/></c:if></td>
                    <td class='center'><c:if test='${admProcessDetails.isMandatory}'><spring:message
                            code='default.boolean.true' text='YES'/></c:if><c:if
                            test='${!admProcessDetails.isMandatory}'><spring:message code='default.boolean.false'
                                                                                     text='NO'/></c:if></td>
                    <td><c:out value='${admProcessDetails.defaultVal}'/></td>
                    <td><c:out value='${admProcessDetails.helpText}'/></td>

                    <td>
                        <a href='${context}/admProcessDetail/edit/<c:out value='${admProcessDetails.id}'/>'><spring:message
                                code='default.button.edit.label'/></a></td>
                    <td><a href='${context}/admProcessDetail/delete/<c:out value='${admProcessDetails.id}'/>'
