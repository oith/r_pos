...
<head>
    <title><spring:message code='default.button.list.label' text='List'/>&nbsp;<spring:message code='authUser'
...
<div class="content-wrapper"><!-- Content Wrapper. Contains page content -->
    ...
    <section class='content-header'><!-- Content Header (Page header) -->
        <h1><spring:message code='default.button.list.label' text='List'/>&nbsp;<spring:message code='authUser'
                                                                                                text='Auth User'/>&nbsp;
        ...
        <ul class='top-links'>
            <sec:authorize url='/authUser/create'>
                <li>
                    <a href='${context}/authUser/create' class='btn btn-primary btn-xs'><i
                    ...
                    <c:if test='${not empty authUsers}'>
                        ...
                        <c:forEach items='${authUsers}' var='authUser' varStatus='loopStatus'>
                            <tr class='${loopStatus.index % 2 == 0 ? 'odd' : 'even'}'>
                                ...
                                <td class='center'><fmt:formatDate value='${authUser.dob}' type='date'
                                                                   pattern='dd/MM/yyyy'/></td>
                                <td class='center'><fmt:formatDate value='${authUser.doj}' type='date'
                                                                   pattern='dd/MM/yyyy'/></td>
                                ...
                                <td class='center'><c:if test='${authUser.enabled}'><spring:message
                                        code='default.boolean.true' text='YES'/></c:if><c:if
                                        test='${!authUser.enabled}'><spring:message code='default.boolean.false'
                                                                                    text='NO'/></c:if></td>
                                <td class='center'><c:if test='${authUser.accountNonExpired}'><spring:message
                                        code='default.boolean.true' text='YES'/></c:if><c:if
                                        test='${!authUser.accountNonExpired}'><spring:message
                                        code='default.boolean.false' text='NO'/></c:if></td>
                                <td class='center'><c:if test='${authUser.accountNonLocked}'><spring:message
                                        code='default.boolean.true' text='YES'/></c:if><c:if
                                        test='${!authUser.accountNonLocked}'><spring:message
                                        code='default.boolean.false' text='NO'/></c:if></td>
                                <td class='center'><c:if test='${authUser.credentialsNonExpired}'><spring:message
                                        code='default.boolean.true' text='YES'/></c:if><c:if
                                        test='${!authUser.credentialsNonExpired}'><spring:message
                                        code='default.boolean.false' text='NO'/></c:if></td>
                                <td class='center'><c:if test='${authUser.openInNewPage}'><spring:message
                                        code='default.boolean.true' text='YES'/></c:if><c:if
                                        test='${!authUser.openInNewPage}'><spring:message code='default.boolean.false'
                                                                                          text='NO'/></c:if></td>
                                ...
                                <td class='center action'>
                                    ...
                                    <sec:authorize url='/authUser/edit'>
                                        <a class='btn btn-primary btn-xs'
                                           href='${context}/authUser/edit/${authUser.id}'>
                                    ...
                                    <sec:authorize url='/authUser/copy'>
                                        <a class='btn btn-warning btn-xs'
                                           href='${context}/authUser/copy/${authUser.id}'>
                                            <i class='fa fa-clone' aria-hidden='true'></i>&nbsp;<spring:message
                                                code='default.button.copy.label'/>
                                        </a>
                                    </sec:authorize>
                                    <sec:authorize url='/authUser/delete'>
                                        <a class='btn btn-danger btn-xs'
                                           href='${context}/authUser/delete/${authUser.id}'
