                                ...
                                <c:if test='${not empty admParams}'>
                                    ...
                                    <c:forEach items='${admParams}' var='admParam'  varStatus='loopStatus'>
                                        <tr class='${loopStatus.index % 2 == 0 ? 'odd' : 'even'}'>
                                            <td><c:out value='${admParam.code}'/></td>
                                            <td><c:out value='${admParam.fullName}'/></td>
                                            <td><c:out value='${admParam.fullNameNative}'/></td>
                                            <td class='center'><c:out value='${admParam.widgetType}'/></td>
                                            <td class='center'><c:if test='${admParam.isActive}'><spring:message code='default.boolean.true' text='YES'/></c:if><c:if test='${!admParam.isActive}'><spring:message code='default.boolean.false' text='NO'/></c:if></td>
                                            <td class='center'><c:if test='${admParam.isMandatory}'><spring:message code='default.boolean.true' text='YES'/></c:if><c:if test='${!admParam.isMandatory}'><spring:message code='default.boolean.false' text='NO'/></c:if></td>
                                            <td class='right'><c:out value='${admParam.slNo}'/></td>
                                            ...
                                            <td class='center action'>
                                                <sec:authorize url='/admParam/show'>
                                                    <a class='btn btn-info btn-xs' href='${context}/admParam/show/${admParam.id}'>
                                                        <i class='fa fa-info-circle'></i>
                                                        <spring:message code='default.button.show.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/admParam/edit'>
                                                    <a class='btn btn-primary btn-xs' href='${context}/admParam/edit/${admParam.id}'>
                                                        <i class='fa fa-edit'></i>
                                                        <spring:message code='default.button.edit.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/admParam/copy'>
                                                    <a class='btn btn-warning btn-xs' href='${context}/admParam/copy/${admParam.id}'>
                                                        <i class='fa fa-clone' aria-hidden='true'></i>
                                                        <spring:message code='default.button.copy.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/admParam/delete'>
                                                    <a class='btn btn-danger btn-xs' href='${context}/admParam/delete/${admParam.id}' onclick="return confirm('${confirmToDelete}');">
