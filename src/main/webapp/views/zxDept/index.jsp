                ...
                <div class='box box-primary'>   
                    ...
                    <div class='box-body'>
                        <table class='table dt-default table-bordered table-striped table-hover table-condensed display nowrap th-center'>
                            ...
                            <tbody>
                                <c:if test='${not empty zxDepts}'>
                                    <spring:message code='default.button.delete.confirm.message' text='Are you sure to delete?' var='confirmToDelete'/>
                                    <c:forEach items='${zxDepts}' var='zxDept'  varStatus='loopStatus'>
                                        <tr class='${loopStatus.index % 2 == 0 ? 'odd' : 'even'}'>
                                            <td><c:out value='${zxDept.code}'/></td>
                                            <td class='center'><c:if test='${zxDept.isActive}'><spring:message code='default.boolean.true' text='YES'/></c:if><c:if test='${!zxDept.isActive}'><spring:message code='default.boolean.false' text='NO'/></c:if></td>
                                            <td class='right'><c:out value='${zxDept.slNo}'/></td>
                                            <td><c:out value='${zxDept.title}'/></td>
                                            <td><c:out value='${zxDept.titleNative}'/></td>
                                            <td><c:out value='${zxDept.remarks}'/></td>

                                            <td class='center action'>
                                                <sec:authorize url='/zxDept/show'>
                                                    <a class='btn btn-info btn-xs' href='${context}/zxDept/show/${zxDept.id}'>
                                                        <i class='fa fa-info-circle'></i>
                                                        <spring:message code='default.button.show.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/zxDept/edit'>
                                                    <a class='btn btn-primary btn-xs' href='${context}/zxDept/edit/${zxDept.id}'>
                                                        <i class='fa fa-edit'></i>
                                                        <spring:message code='default.button.edit.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/zxDept/copy'>
                                                    <a class='btn btn-warning btn-xs' href='${context}/zxDept/copy/${zxDept.id}'>
                                                        <i class='fa fa-clone' aria-hidden='true'></i>
                                                        <spring:message code='default.button.copy.label'/>
                                                    </a>
                                                </sec:authorize>
                                                <sec:authorize url='/zxDept/delete'>
                                                    <a class='btn btn-danger btn-xs' href='${context}/zxDept/delete/${zxDept.id}' onclick="return confirm('${confirmToDelete}');">
