...
<spring:message code='admMenuCommons' text='Adm Menu Commons'/> | <a href='${context}/admMenuCommon/create'>
    <spring:message code='default.create.link.label'/>&NonBreakingSpace;<spring:message code='AdmMenuCommon'
                                                                                        text='Adm Menu Common'/></a>
        ...
        <table class='table table-bordered table-striped table-hover table-condensed dt-responsive display nowrap th-center'>
            ...
            <c:forEach items='${admSubModule.admMenuCommons}' var='admMenuCommons' varStatus='loopStatus'>
                <tr class='${loopStatus.index % 2 == 0 ? 'odd' : 'even'}'>
                    <td><a href='${context}/admMenuCommon/show/<c:out value='${admMenuCommons.id}'/>'><spring:message
                    ...
                    <td class='center'><c:if test='${admMenuCommons.isActive}'><spring:message
                            code='default.boolean.true' text='YES'/></c:if><c:if
                            test='${!admMenuCommons.isActive}'><spring:message code='default.boolean.false' text='NO'/></c:if></td>
                    ...
                    <td class='center'><c:if test='${admMenuCommons.isExternal}'><spring:message
                            code='default.boolean.true' text='YES'/></c:if><c:if
                            test='${!admMenuCommons.isExternal}'><spring:message code='default.boolean.false'
                                                                                 text='NO'/></c:if></td>
                    <td class='center'><c:if test='${admMenuCommons.isOpenInNewTab}'><spring:message
                            code='default.boolean.true' text='YES'/></c:if><c:if
                            test='${!admMenuCommons.isOpenInNewTab}'><spring:message code='default.boolean.false'
                                                                                     text='NO'/></c:if></td>

                    <td><a href='${context}/admMenuCommon/edit/<c:out value='${admMenuCommons.id}'/>'><spring:message
                            code='default.button.edit.label'/></a></td>
                    <td><a href='${context}/admMenuCommon/delete/<c:out value='${admMenuCommons.id}'/>'
