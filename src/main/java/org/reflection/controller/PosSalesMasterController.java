...
@Controller
@RequestMapping(value = "/posSalesMaster")

public class PosSalesMasterController extends _BaseController {

    protected static final String MODEL = "posSalesMaster";
    ...
    String getCodableDTO(@RequestParam(value = "code") String code) {
        try {
            PosSalesMaster codable = posSalesMasterService.findByCode(code);
            return codable.getRemarks();
        } catch (Exception e) {
            System.out.println("mac err codable PosSalesMaster Not Found: " + e);
            return "Not Found";
        }
    }


    private void commonPost(PosSalesMaster currObject) {
        try {
    ...
    public String create(ModelMap model) {
        model.addAttribute(MODEL, new PosSalesMaster());
    ...
    @PostMapping("/create")
    public String save(
            @ModelAttribute(MODEL) @Valid PosSalesMaster currObject,
        ...
        if (!bindingResult.hasErrors()) {
            try {
                PosSalesMaster posSalesMaster = posSalesMasterService.create(currObject);
                posSalesMaster.setId(faker(posSalesMaster.getId()));
                addFeedbackMessage(redirectAttributes, FLASH_SUCCESS, FEEDBACK_MESSAGE_KEY_CREATED, posSalesMaster.getCode());
                return "redirect:/" + SHOW + "/" + posSalesMaster.getId();
            } catch (Exception e) {
    ...
    public String edit(@PathVariable("id") BigInteger id, ModelMap model, RedirectAttributes redirectAttributes) {
        ...
        try {
            PosSalesMaster posSalesMaster = posSalesMasterService.findById(faker(id));
            posSalesMaster.setId(faker(posSalesMaster.getId()));
            model.addAttribute(MODEL, posSalesMaster);
            model.addAttribute("_action", "edit");
            return FORM;
        } catch (PosSalesMasterNotFoundException e) {
    ...
    public String update(
        ...
        if (!bindingResult.hasErrors()) {
            try {
                PosSalesMaster posSalesMaster = posSalesMasterService.update(currObject);
                addFeedbackMessage(redirectAttributes, FLASH_SUCCESS, FEEDBACK_MESSAGE_KEY_EDITED, posSalesMaster.getCode());
    ...
    public String copied(
        ...
        if (!bindingResult.hasErrors()) {
            try {
                PosSalesMaster posSalesMaster = posSalesMasterService.copy(currObject);
                posSalesMaster.setId(faker(posSalesMaster.getId()));
                addFeedbackMessage(redirectAttributes, FLASH_SUCCESS, FEEDBACK_MESSAGE_KEY_COPIED, posSalesMaster.getCode());
                return "redirect:/" + SHOW + "/" + posSalesMaster.getId();
            } catch (Exception e) {
                errorHandler(bindingResult, e);
            }
    ...
    public String search(@ModelAttribute(SEARCH_CRITERIA) _SearchDTO searchCriteria, ModelMap model) {
        ...
        List<PosSalesMaster> posSalesMasters;
   
        if (searchTerm != null && !searchTerm.trim().isEmpty()) {
            posSalesMasters = posSalesMasterService.search(searchCriteria);
        } else {
            posSalesMasters = posSalesMasterService.findAll(searchCriteria);
        }
        model.addAttribute(MODELS, posSalesMasters);
        ...
        */
        Iterable<PosSalesMaster> posSalesMasters = posSalesMasterService.findAll();
        for (PosSalesMaster obj : posSalesMasters) {
    ...
    public String index(ModelMap model) {
        ...
        List<PosSalesMaster> posSalesMasters = posSalesMasterService.findAll(searchCriteria);
    ...
    public String delete(@PathVariable("id") BigInteger id, RedirectAttributes redirectAttributes) {
        ...
        try {
            PosSalesMaster deleted = posSalesMasterService.delete(faker(id));
